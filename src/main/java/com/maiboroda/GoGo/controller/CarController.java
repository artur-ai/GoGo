...
public class CarController {
    ...
    public ResponseEntity<CarResponseDto> addCarAsAdmin(@Valid @RequestBody CarRequestDto carRequestDto) {
        Authentication auth = SecurityContextHolder.getContext().getAuthentication();
        String adminEmail = auth.getName();

        log.info("Admin {} is adding new car: {} {}", adminEmail, carRequestDto.getBrand(), carRequestDto.getModel());
        CarResponseDto responseDto = carService.addCarWithNotification(carRequestDto, adminEmail);
        return ResponseEntity.status(HttpStatus.CREATED).body(responseDto);
    }
